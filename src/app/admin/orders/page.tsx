        ...
        if (response.status === 200) {
          ...
          const ordersWithDetails = await Promise.all(
            ordersData.map(async (order: any) => {
              const userResponse = await axios.post('/api/getUsers', { userId: [order.user] });
              const powerPlantResponse = await axios.post('/api/getPowerPlant', { powerPlantIds: [order.powerPlant] });
              
              
              if (userResponse.status === 200 && powerPlantResponse.status === 200) {
    ...
    <div className="flex flex-col items-center justify-center min-h-screen py-2">
      ...
      {filteredOrders.length > 0 ? (
        filteredOrders.map((order) => (
          <Card key={order._id} className="mb-4 w-full max-w-2xl">
            <CardHeader>
            ...
            <CardContent>
              ...
              <h3 className="text-lg font-bold mt-4">User Details</h3>
            ...
            <CardFooter>
              <div className='flex justify-between items-center gap-2'>
                <Select onValueChange={(value) => handleStatusChange(order._id, value)}>
                  <div className={`p-2 rounded w-80 ${getStatusColor(order.orderStatus)}`}>
